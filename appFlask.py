...
def insertar_dueno():
    data = request.get_json()  # Obtener datos JSON enviados desde Postman
...
def insertar_servicio():
    ...
    connection = mysql.connector.connect(**db_config)
    cursor = connection.cursor()
    sql = "INSERT INTO servicio (NOMBRE, PRECIO, DESCRIPCION) VALUES (%s, %s, %s)"
    values = (data['NOMBRE'], data['PRECIO'], data['DESCRIPCION'])
    cursor.execute(sql, values)
    connection.commit()
    cursor.close()
    connection.close()
...
def actualizar_servicio(id):
    ...
    connection = mysql.connector.connect(**db_config)
    cursor = connection.cursor()
    sql = "UPDATE servicio SET NOMBRE = %s, PRECIO = %s, DESCRIPCION = %s WHERE ID_SERVICIO = %s"
    values = (data['NOMBRE'], data['PRECIO'], data['DESCRIPCION'], id)
    cursor.execute(sql, values)
    connection.commit()
    cursor.close()
    connection.close()
